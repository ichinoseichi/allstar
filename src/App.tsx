...
type Player = { id: string; room_code: string; display_name: string; total_score: number };
...
function useRoomCore() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "rounds", filter: `room_code=eq.${roomCode}` },
        () => refreshRounds()
      )
      .on(
        "postgres_changes",
...
function GMPage() {
  ...
  async function newRound() {
    ...
    if (current && current.status !== "scored") {
      return alert("前の問題が完結していません（締切＆集計 → 正解者に加点 まで実施）");
    }
  ...
  async function closeRound() {
    if (!current) return;
    const { error } = await supabase.from("rounds").update({ status: "closed" }).eq("id", current.id);
  ...
  async function setCorrect(c: "A" | "B" | "C" | "D") {
    if (!current) return;
    const { error } = await supabase.from("rounds").update({ correct_choice: c }).eq("id", current.id);
  ...
  async function resetScores() {
    ...
    const { error } = await supabase.rpc("reset_scores", { p_room_code: roomCode });
  ...
  return (
    <PageWrapper>
      ...
      {!room && (
        <Grid2>
          <Box title="GM：ルーム作成">
            <button style={btnPrimary} onClick={createRoom}>新しいルームを作る（4桁数字）</button>
            {roomCode && <div style={{ marginTop: 8, fontSize: 12 }}>作成されたルーム番号: <b>{roomCode}</b></div>}
            <div style={{ marginTop: 10, fontSize: 12, color: "#333" }}>※ パスワード不要。番号を配布してください。</div>
          </Box>
          ...
          <Box title={`参加者 / 累計スコア（ルーム ${roomCode})`}>
            {players.length === 0 ? (
              ...
              <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                {players.map((p) => (
                  <li key={p.id} style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid #eee", padding: "6px 0" }}>
        ...
        <Grid2 style={{ marginTop: 12 }}>
          <Box title="GM 操作">
            <div style={{ display: "grid", gap: 8 }}>
              <div>現在の問題：{current ? `#${current.index_no} [${current.status}]` : "なし"}</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {!current || current.status === "scored" ? <button style={btnPrimary} onClick={newRound}>新しい問題</button> : null}
                {current && current.status === "ready" && <button style={btnBlue} onClick={openRound}>回答受付開始</button>}
                {current && current.status === "open" && <button style={btnDanger} onClick={closeRound}>締切＆集計</button>}
              </div>

              {(current && (current.status === "closed" || current.correct_choice || current.status === "scored")) && (
                <div style={panel}>
                  <div>
                    正解を選択：
                    {(["A", "B", "C", "D"] as const).map((c) => (
                      <button
                        key={c}
                        style={{ ...btn, ...(current?.correct_choice === c ? btnGreen : {}), marginLeft: 6 }}
                  ...
                  <div style={{ marginTop: 10, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    <input style={inpN} type="number" value={w1} onChange={(e) => setW1(Number(e.target.value))} />
                    <input style={inpN} type="number" value={w2} onChange={(e) => setW2(Number(e.target.value))} />
                    <input style={inpN} type="number" value={wOther} onChange={(e) => setWOther(Number(e.target.value))} />
                  </div>

                  {/* 発表：正解者のみランキング（開始合図が来るまで非表示） */}
                  <div style={{ marginTop: 6 }}>
                    <div style={{ fontWeight: 700, marginBottom: 6 }}>正解者ランキング発表</div>

                    <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
                      <button
                        style={btn}
                        onClick={startReveal}
                        disabled={!current?.correct_choice || !ranks.length || !!current?.reveal_started}
                      >
                        🎉 発表を開始（下から）
                      </button>
                      <span style={{ fontSize: 12, color: "#333" }}>
                        {!!current?.reveal_started ? "※ 発表中" : "※ 押すまで非表示（参加者画面も同時に切替）"}
                      </span>
                    </div>

                    {(current?.correct_choice && !!current?.reveal_started) ? (
                      (ranks.filter(r => r.choice === current.correct_choice).length === 0) ? (
                        <div style={{ fontSize: 12, color: "#333" }}>正解者なし</div>
                      ) : (
                        <ol style={{ paddingLeft: 18, margin: 0 }}>
                          {/* 下から発表：末尾から revealShown 人だけフェードイン */}
                          {ranks.filter(r => r.choice === current.correct_choice).map((r, idx, arr) => {
                            const fromBottomIndex = arr.length - 1 - idx;
                            const visible = fromBottomIndex < (revealActive ? revealShown : 0);
                            ...
                            return (
                              ...
                              >
                                ...
                                {typeof r.elapsed_sec === "number" ? `：${r.elapsed_sec.toFixed(1)} 秒` : ""}
                              </li>
                            );
                          })}
                        </ol>
                      ...
                      <div style={{ fontSize: 12, color: "#333" }}>※ 発表はまだ開始されていません</div>
                    )}
                  </div>

                  <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
                    <button
                      style={{ ...btn, ...btnGreen, opacity: (!current?.correct_choice || isScoring) ? 0.6 : 1 }}
                      disabled={!current?.correct_choice || isScoring || current?.status === "scored"}
                      onClick={applyScores}
                    >
                      正解者に加点
                    </button>
                    {current?.status === "scored" && <button style={{ ...btn }} onClick={newRound}>次の問題へ</button>}
                  </div>
                </div>
              )}

              <div style={{ marginTop: 8 }}>
                <button style={btn} onClick={resetScores}>スコアリセット（GM）</button>
              </div>
            </div>
          </Box>
          ...
          <Box title="配布用リンク">
            <div style={{ display: "grid", gap: 8 }}>
              <div style={{ fontSize: 14 }}>
                参加者は <code>/player</code> にアクセスし、ルーム番号 <b>{roomCode || "----"}</b> を入力してください。
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <a href="/gm" style={{ ...btn, textDecoration: "none" }}>GMページを開く</a>
                <a href="/player" style={{ ...btn, textDecoration: "none" }}>参加者ページを開く</a>
              </div>
            </div>
          </Box>
        </Grid2>
...
function PlayerPage() {
  ...
  // 自分のこのラウンドでの回答（枠を保持）
  const [myAnswerChoice, setMyAnswerChoice] = useState<"A"|"B"|"C"|"D"|null>(null);
  ...
  async function joinAsPlayer() {
    if (!roomCode || !/^\d{4}$/.test(roomCode)) return alert("4桁の数字のルーム番号を入力してください");
  ...
  async function answer(choice: "A" | "B" | "C" | "D") {
    ...
    if (myAnswerChoice) return; // 既に押していれば無視

    const payload = { room_code: me.room_code, round_id: current.id, player_id: me.id, choice };
    const { error } = await supabase.from("answers").insert(payload);
    if (error) {
      // @ts-ignore 23505 (unique_violation) は無視
  ...
  const showAnswerColor = !!current?.correct_choice && (current?.status === "closed" || current?.status === "scored");
  ...
  return (
    <PageWrapper>
          ...
          <Box title="参加フォーム">
            <div style={{ display: "grid", gap: 8 }}>
              <input
                ...
                onChange={(e) => setRoomCode(e.target.value.replace(/\D/g, "").slice(0, 4))}
              />
              <input style={inp} placeholder="表示名" value={myName} onChange={(e) => setMyName(e.target.value)} />
              <button style={btn} onClick={joinAsPlayer}>参加</button>
            </div>
          </Box>
        </Grid1>
      )}
      ...
      {me && current && (
        <Grid1>
          <Box title={`問題 #${current.index_no}：${current.status.toUpperCase()}`}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {(["A", "B", "C", "D"] as const).map((c) => {
                const isCorrect = showAnswerColor && current.correct_choice === c;
                ...
                if (isMine) {
                  style = {
                    ...style,
                    boxShadow: "0 0 0 3px rgba(17,24,39,1), 0 0 0 6px rgba(17,24,39,0.15)",
                    outline: "none",
                  };
                }

                return (
                  <button key={c} onClick={() => answer(c)} disabled={current.status !== "open"} style={style}>
                    {c}
                  </button>
                );
              })}
            ...
            {/* 発表：GMが開始するまでは非表示。開始されたら下からゆっくり表示 */}
    ...
    <PageWrapper>
      ...
      <Grid2>
        <Box title="GM（司会者）の方はこちら">
          <a href="/gm" style={{ ...btnPrimary, textDecoration: "none", display: "inline-block" }}>🎛️ GMページへ</a>
          <div style={{ fontSize: 12, color: "#333", marginTop: 8 }}>
            ルーム番号の作成、回答受付の開始/締切、正解設定、発表開始、採点、スコアリセットを行います。
          </div>
        </Box>
        <Box title="参加者の方はこちら">
          <a href="/player" style={{ ...btn, textDecoration: "none", display: "inline-block" }}>👤 参加者ページへ</a>
          <div style={{ fontSize: 12, color: "#333", marginTop: 8 }}>
...
const Grid1: React.FC<{ children: React.ReactNode; style?: React.CSSProperties }> = ({ children, style }) => (
  <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 12, ...style }}>{children}</div>
);

const Grid2: React.FC<{ children: React.ReactNode; style?: React.CSSProperties }> = ({ children, style }) => (
